...
/// basic tests [p]
	...
	QA.ASSERT( new Iconv('UTF-8', 'ISO-8859-1').process('é').length, 2, '"é" char length in UTF-8' );

	var conv = new Iconv('UTF-8', 'ISO-8859-1');
	var invConv = new Iconv('ISO-8859-1', 'UTF-8');
	var converted = conv.process('été');
...
/// invalid sequense detection [p]
	...
	var utf8 = (new Iconv('UTF-8', 'ISO-8859-1')).process('été').split('');
...
/// store UCS-2le to JS unicode (3) [p]
  ...
  QA.ASSERT_STR( conv.process(result), 'été à la plage', 'check the resulting string' );
