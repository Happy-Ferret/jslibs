...
DEFINE_FUNCTION( valueOf ) {
	...
	JL_CHK( jl::setValue(cx, JL_RVAL, pv->effectSlot) );
	return true;
	JL_BAD;
}
...
DEFINE_PROPERTY_SETTER( effect ) {
	...
	ALuint effect;
	if ( !vp.isUndefined() )
		JL_CHK( jl::getValue(cx, vp, &effect) );
	else
...
DEFINE_PROPERTY_SETTER( effectSendAuto ) {
	...
	JL_ASSERT_THIS_OBJECT_STATE( pv );
	bool sendAuto;
	JL_CHK( jl::getValue(cx, vp, &sendAuto) );
...
DEFINE_PROPERTY_GETTER( effectSendAuto ) {
	...
	JL_CHK( CheckThrowCurrentOalError(cx) );
	JL_CHK(jl::setValue(cx, vp, sendAuto == AL_TRUE ? true : false) );
	return true;
	JL_BAD;
}
