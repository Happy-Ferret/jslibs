...
DEFINE_PROPERTY_SETTER( kinematic ) {
	...
	JL_ASSERT_THIS_OBJECT_STATE( bodyId );
	bool kinematic;
	JL_CHK( jl::getValue(cx, *vp, &kinematic) );
	if ( kinematic )
...
DEFINE_PROPERTY_SETTER( autoDisable ) {
	...
	JL_ASSERT_THIS_OBJECT_STATE( bodyId );
	bool autoDisable;
	JL_CHK( jl::getValue(cx, *vp, &autoDisable) );
...
DEFINE_PROPERTY_SETTER( gravityMode ) {
	...
	JL_ASSERT_THIS_OBJECT_STATE( bodyId );
	bool gravityMode;
	JL_CHK( jl::getValue(cx, *vp, &gravityMode) );
...
DEFINE_PROPERTY_SETTER( maxAngularSpeed ) {
	...
	JL_CHK( jl::getValue(cx, *vp, &maxAngularSpeed) );
...
DEFINE_PROPERTY_SETTER( linearDamping ) {
	...
	JL_ASSERT_THIS_OBJECT_STATE( bodyId );
	ode::dReal scale;
	JL_CHK( jl::getValue(cx, *vp, &scale) );
...
DEFINE_PROPERTY_SETTER( linearDampingThreshold ) {
	...
	JL_ASSERT_THIS_OBJECT_STATE( bodyId );
	ode::dReal threshold;
	JL_CHK( jl::getValue(cx, *vp, &threshold) );
...
DEFINE_PROPERTY_GETTER( mass ) {
	...
	JSObject *massObject = jl::newObjectWithGivenProto(cx, JL_CLASS(Mass), JL_CLASS_PROTOTYPE(cx, Mass));
