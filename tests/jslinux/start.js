...
var url = [ 'bellard.org', 80, '/jslinux/' ];
var files = [ 'cpux86-ta.js', 'jslinux.js', 'linuxstart.bin', 'root.bin', 'vmlinux26.bin' ];

for each ( var fileName in files ) {
	
	var f = new File(fileName);
	if ( !f.exist )
		f.content = DownloadFile(url[0], url[1], url[2]+fileName);
}
